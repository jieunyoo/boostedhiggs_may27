...
`   """
    ...
    #histBins = [40,60,80,100,120,180]
    histBins = [40,60,80,100,120,140,160,180]
    hists = hist2.Hist(
        hist2.axis.StrCategory([], name="Sample", growth=True),
        hist2.axis.StrCategory([], name="Systematic", growth=True),
        hist2.axis.StrCategory([], name="Region", growth=True),
        hist2.axis.Variable( histBins, name="mass_observable", label=r"V softdrop mass [GeV]", overflow=False,
        ),
        ...
        for ch in channels:  # e.g. mu, ele
            ...
            for sample in os.listdir(samples_dir[year]):
                ...
                for region, region_sel in regions_sel.items():  # e.g. pass, fail, top control region, etc.
                    ...
                    hists.fill(Sample=sample_to_use, Systematic="nominal", Region=region, mass_observable=df["fj_mass"], weight=nominal, )
...
#june 29th 3:58 pm, add farouk's systematic for reweighting
                    ...
                    hists.fill( Sample=sample_to_use, Systematic="weight_pdf_acceptance_up", Region=region, mass_observable=df["fj_mass"], weight=shape_up, )

                    hists.fill( Sample=sample_to_use, Systematic="weight_pdf_acceptance_down", Region=region, mass_observable=df["fj_mass"], weight=shape_down, )
                    ...
                    hists.fill( Sample=sample_to_use, Systematic="weight_qcd_scale_up", Region=region, mass_observable=df["fj_mass"], weight=shape_up, )

                    hists.fill( Sample=sample_to_use, Systematic="weight_qcd_scale_down", Region=region, mass_observable=df["fj_mass"], weight=shape_down,  )

                    # ------------------- Some JECs -------------------
                    for syst, (yrs, smpls, var) in { **JEC_systs_uncorrelated}.items():
                        if (sample_to_use in smpls) and (year in yrs) and (ch in var):
